...
import {
  ...
  Sparkles, BookOpen, Filter, TrendingUp,
} from 'lucide-react';
          ...
          <div className="container mx-auto px-4 relative z-10 py-16 md:py-24">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
              ...
              <Badge className="bg-legal-gold/10 text-legal-gold border-legal-gold/20 px-4 py-1.5 text-xs rounded-full">
                ...
                مقالات محدّثة باستمرار
              </Badge>
              <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-foreground leading-[1.15]">
                المدونة <span className="bg-gradient-to-l from-primary to-legal-gold bg-clip-text text-transparent">القانونية</span>
              </h1>
              <p className="text-muted-foreground max-w-xl mx-auto text-sm sm:text-base leading-relaxed">
                محتوى قانوني مغربي معمّق ومحدّث، مكتوب بأسلوب واضح يساعدك على فهم حقوقك
              </p>
              ...
              <div className="relative max-w-lg mx-auto">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="pr-12 h-13 text-base rounded-2xl border-border/40 bg-card shadow-lg shadow-foreground/[0.03] focus:shadow-xl transition-shadow"
                  />
                </div>
              </div>
            </motion.div>
        ...
        <main className="container mx-auto px-4 py-10 md:py-14">
          {loading ? (
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <p className="text-sm text-muted-foreground">{search ? 'جرّب كلمات مفتاحية مختلفة' : 'لا توجد مقالات في هذا التصنيف بعد'}</p>
            </motion.div>
          ) : (
            <>
              {/* Featured */}
                    ...
                    <div className="relative rounded-3xl overflow-hidden border border-border/15 bg-card shadow-xl shadow-foreground/[0.03] hover:shadow-2xl transition-all duration-500">
                      <div className="grid grid-cols-1 lg:grid-cols-2">
                        <div className="relative h-64 lg:h-96 overflow-hidden">
                          ...
                          <Badge className="absolute top-4 right-4 bg-legal-gold/90 text-primary-foreground border-0 px-3 py-1 text-[10px] rounded-full shadow-lg gap-1.5">
                            <TrendingUp className="h-3 w-3" /> مقال مميّز
                          </Badge>
                        </div>
                        <div className="p-7 lg:p-10 flex flex-col justify-center space-y-5">
                          <Badge variant="outline" className="w-fit rounded-full px-3 py-1 text-[10px]">
                ...
                {(search || activeCategory !== 'الكل' ? filtered : filtered.slice(1)).map((article, i) => (
                  <AnimatedCard key={article.id} index={i}>
                    <Link to={`/blog/${article.slug}`} className="group block h-full">
                      <div className="relative h-full rounded-2xl border border-border/15 bg-card overflow-hidden hover:shadow-xl hover:border-primary/10 transition-all duration-500 hover:-translate-y-1.5">
                        ...
                        <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-gradient-to-l from-transparent via-primary/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                      </div>
                    </Link>
                  </AnimatedCard>
                ))}
