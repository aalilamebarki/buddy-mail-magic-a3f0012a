...
import { ChevronRight, ChevronLeft, CalendarDays, Clock, MapPin, Gavel, CalendarRange, Calendar as CalendarIcon } from 'lucide-react';
...
const CalendarView = ({ sessions }: CalendarViewProps) => {
  ...
  const renderSessionCard = (s: SessionRecord, compact = false) => {
      ...
      {weekDays.map(day => {
        ...
        return (
          <div key={key} className="space-y-2">
            <div
              className={cn("text-center p-2 rounded-lg cursor-pointer hover:bg-muted/50", isToday_ && "bg-primary/10")}
              onClick={() => { setCurrentDate(day); setViewMode('day'); }}
            >
              <div className="text-xs text-muted-foreground">{format(day, 'EEEE', { locale: ar })}</div>
              <div className={cn("text-lg font-bold mx-auto w-8 h-8 flex items-center justify-center rounded-full", isToday_ && "bg-primary text-primary-foreground")}>
                {format(day, 'd')}
              </div>
            </div>
        ...
        <CardContent className="pt-4 pb-3">
          <div className="flex items-center justify-between gap-2 flex-wrap">
            <div className="flex gap-1 bg-muted rounded-lg p-1">
              <Button variant={viewMode === 'month' ? 'default' : 'ghost'} size="sm" className="gap-1 h-8 text-xs" onClick={() => setViewMode('month')}>
                <CalendarIcon className="h-3.5 w-3.5" /> شهري
              </Button>
              <Button variant={viewMode === 'week' ? 'default' : 'ghost'} size="sm" className="gap-1 h-8 text-xs" onClick={() => setViewMode('week')}>
                <CalendarRange className="h-3.5 w-3.5" /> أسبوعي
              </Button>
              <Button variant={viewMode === 'day' ? 'default' : 'ghost'} size="sm" className="gap-1 h-8 text-xs" onClick={() => setViewMode('day')}>
            ...
            <div className="flex items-center gap-2">
              ...
              <Button variant="outline" size="sm" className="h-8 text-xs" onClick={() => setCurrentDate(new Date())}>اليوم</Button>
              <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => nav(-1)}>
